...
class GradeController extends Controller
{
	 ...
	 */
	public function actionView($code='',$openid='')
	{
        $openid = empty($openid)?getopenid($code):$openid;
        $user = User::model()->findByAttributes(array('openid'=>$openid));
        if(isset($user))
        {
            $model = Grade::model()->findByAttributes(array('sid'=>$user->sid));
            $this->render('view',array('model'=>$model,'user'=>$user));
            exit;
        }
        p()->user->setFlash('err','请输入学生身份证号，绑定手机');
        $this->redirect(array('/site/band','openid'=>$openid));
	}
